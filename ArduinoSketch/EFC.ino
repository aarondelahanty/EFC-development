...
//This application (sketch) is intended to be the sole sketch responsible for measuring and regulating pressure in the
//Electronic Flow Control (EFC) system being developed for T.Franke's microfluidic group. The objective of the sketch is to faciliate
//communication with two types of integrated chips (i) Analog Microselectronic's AMS5915 pressure sensors,
//and (ii) Microchip Technology Inc.'s MCP4921 DAC.

//In addition to this sketch the system will use a LabView VI to set the desired pressure values by DAC, and display the measured pressure
// for the 4 discrete channels. This communication will be facilated by serial communication over USB.
...
//Comparison is used to determine whether the high range 1000mbar or high resolution 350mbar sensor should be sued
    float p_comp[8];
    float p_out[4];
    ...
    for (int i = 0; i <= 3; i++)
       {
        ...
        //Shift the config bits 12 positions leftward, augment with the a[i] pressure value in volts
